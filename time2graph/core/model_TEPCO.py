...
class Time2Graph(ModelUtils):
    ...
    def extract_features(self, X,Z, init=0, train=False,mode ='all'):
        """
        @param X:
    ...
    def fit(self, X, Y,Z, n_splits=5, balanced=True, cache_dir='{}/scripts/cache/'.format(module_path), **kwargs):
        """
        @param X:
            ndarray with shape (N x L x data_size), input time series.
        @param Y:
